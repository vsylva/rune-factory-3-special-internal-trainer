...
use windows::Win32::{
    ...
    UI::{
        Input::{
            GetRawInputData, HRAWINPUT, KeyboardAndMouse::*, MOUSE_MOVE_ABSOLUTE, RAWINPUT,
            RAWINPUTHEADER, RAWKEYBOARD, RAWMOUSE, RID_DEVICE_INFO_TYPE, RID_INPUT,
...
fn handle_input(io: &mut Io, state: u32, WPARAM(wparam): WPARAM, LPARAM(lparam): LPARAM) {
    ...
    io.add_key_event(Key::LeftCtrl, is_vk_down(VK_CONTROL));
    io.add_key_event(Key::LeftShift, is_vk_down(VK_SHIFT));
    io.add_key_event(Key::LeftAlt, is_vk_down(VK_MENU));
    io.add_key_event(Key::LeftSuper, is_vk_down(VK_APPS));
