...
pub(crate) unsafe fn create_hook(mod_addr: usize, mod_size: usize) {
    COIN_ADDR = (crate::MOD_SANDLL.base + 0x2AD192C) as *mut u32;
    WOOD_ADDR = (crate::MOD_SANDLL.base + 0x2AD1930) as *mut u16;
...
impl AsmHook {
    ...
    pub unsafe fn create(
        ...
        self.target_addr = libmem::sig_scan(sig, addr, size).unwrap();

        let back_addr = self.target_addr + occupied;
        ...
        libmem::write_memory_ex(
            &libmem::find_process("Rune Factory 3 Special.exe").unwrap(),
            detour_addr.byte_add(end_offset) as usize,
            back_shell_code.as_slice(),
        )
        .unwrap();
    ...
    pub fn switch(&mut self) {
        if self.is_enabled {
            minhook_raw::enable_hook(self.target_addr as *mut core::ffi::c_void);
        } else {
            minhook_raw::disable_hook(self.target_addr as *mut core::ffi::c_void);
        }
    }
}
